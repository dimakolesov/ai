...
from db import (
    ...
    check_daily_message_limit, increment_daily_message_count, has_lifetime_access,
    grant_lifetime_access, has_access, set_user_language, get_user_language
)
...
async def get_girls_keyboard(user_id: int) -> InlineKeyboardMarkup:
    ...
    age_suffix = "лет" if user_lang == "ru" else "years"
    
    girl_info = {
        "girl_sakura": {"name": locale_manager.get_text("girl_sakura"), "age": f"20 {age_suffix}"},
        "girl_reiko": {"name": locale_manager.get_text("girl_reiko"), "age": f"24 {age_suffix}"},
        "girl_ayane": {"name": locale_manager.get_text("girl_ayane"), "age": f"28 {age_suffix}"},
        "girl_hikari": {"name": locale_manager.get_text("girl_hikari"), "age": f"26 {age_suffix}"},
        "girl_yuki": {"name": locale_manager.get_text("girl_yuki"), "age": f"22 {age_suffix}"}
    }
...
async def age_confirm_handler(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    await callback.message.answer(
        ...
        reply_markup=InlineKeyboardMarkup(
            inline_keyboard=[
                [InlineKeyboardButton(text="🆓 Попробовать бесплатно", callback_data="start_game")],
                [InlineKeyboardButton(text="💎 Купить подписку", callback_data="premium_subscription")]
            ]
        )
    )
...
async def start_game_handler(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    # Предупреждение о пробном периоде
    trial_info = locale_manager.get_text("trial_info")
    
    # Переходим к вводу имени
    await state.set_state(Onboarding.asking_name)
    await callback.message.answer(trial_info)
...
async def mood_handler(message: Message, state: FSMContext) -> None:
    """Обработчик команды /mood - показывает текущее настроение виртуальной подруги."""
...
💫 Совместимость с виртуальной подругой: {compatibility}%
...
async def relationships_handler(message: Message, state: FSMContext) -> None:
    """Обработчик команды /relationships - показывает статус отношений с виртуальной подругой."""

    status_text = await get_relationship_status(message.from_user.id)
    await message.answer(status_text, reply_markup=kb_main_menu())


@router.message(Command("payment_confirmed"))
async def payment_confirmed_handler(message: Message, state: FSMContext) -> None:
    """Обработчик подтверждения оплаты пользователем."""
    ...
    # Логирование
    bot_logger.log_command(message.from_user.id, "/payment_confirmed")
    
    try:
        # Обрабатываем успешный платеж
        result = await payment_processor.process_payment_success(
            user_id=message.from_user.id,
            payment_id=f"manual_confirmation_{message.from_user.id}"
        )
...
💖 Спасибо за покупку! Наслаждайтесь общением!
"""
            ...
            await message.answer(
                f"❌ Ошибка активации подписки: {result.get('error', 'Неизвестная ошибка')}"
            )
            
    except Exception as e:
        bot_logger.log_system_error(e, f"Payment confirmation failed for user {message.from_user.id}")
        await message.answer("❌ Произошла ошибка при подтверждении платежа")
...
async def enter_name_handler(message: Message, state: FSMContext) -> None:
    ...
    await message.answer(
        ...
        reply_markup=await get_girls_keyboard(message.from_user.id)
    )
...
async def chat_handler(message: Message, state: FSMContext) -> None:
    ...
    user_has_access = await has_access(message.from_user.id)
    user_has_lifetime = await has_lifetime_access(message.from_user.id)
    
    # Проверяем пробный период и лимит ежедневных сообщений
    if not user_has_lifetime and user_has_access:
        can_send, current_count, limit = await check_daily_message_limit(message.from_user.id)
        
        if not can_send:
            # Пользователь достиг лимита сообщений
            end_trial_message = locale_manager.get_text("trial_limit_reached")
            
            keyboard = InlineKeyboardMarkup(
                inline_keyboard=[
                    [InlineKeyboardButton(text=locale_manager.get_text("monthly_subscription"), callback_data="buy_monthly_subscription")],
                    [InlineKeyboardButton(text=locale_manager.get_text("lifetime_access"), callback_data="buy_lifetime_access")],
                    [InlineKeyboardButton(text=locale_manager.get_text("back_to_menu"), callback_data="main_menu")]
                ]
            )
            
            await message.answer(end_trial_message, reply_markup=keyboard)
            return
        
        # Увеличиваем счетчик сообщений
        await increment_daily_message_count(message.from_user.id)
    ...
    # Получаем язык пользователя и устанавливаем локаль
...
💖 Статус отношений с виртуальной подругой: {await get_relationship_level_name(callback.from_user.id)}
...
async def premium_subscription_handler(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    try:
        ...
        # Редактируем сообщение с информацией о подписке
        await callback.message.edit_text(
            f"{locale_manager.get_text('premium_subscription')}\n\n"
            f"{locale_manager.get_text('subscription_description')}\n\n"
            f"{locale_manager.get_text('subscription_includes')}\n"
            f"• {locale_manager.get_text('unlimited_messages')}\n"
            f"• {locale_manager.get_text('all_games_access')}\n"
            f"• {locale_manager.get_text('bot_personalization')}\n"
            f"• {locale_manager.get_text('priority_support')}\n"
            f"• {locale_manager.get_text('days_access')}\n\n"
            f"{locale_manager.get_text('stars_info')}\n\n"
            f"{locale_manager.get_text('choose_option')}",
            reply_markup=InlineKeyboardMarkup(
                inline_keyboard=[
                    [InlineKeyboardButton(text=locale_manager.get_text("monthly_subscription"), callback_data="buy_monthly_subscription")],
                    [InlineKeyboardButton(text=locale_manager.get_text("lifetime_access"), callback_data="buy_lifetime_access")],
...
@router.callback_query(F.data == "stars_payment")
@handle_errors
@handle_telegram_errors
async def stars_payment_handler(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    # Логирование
    bot_logger.log_callback(callback.from_user.id, "stars_payment")
    ...
    try:
        ...
        bot_logger.log_system_error(e, f"Failed to process stars payment request for user {callback.from_user.id}")
        await callback.answer("❌ Ошибка обработки запроса", show_alert=True)
...
async def girl_choice_handler(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    if user_lang == "en":
        girl_info = {
            "girl_sakura": {
                "name": locale_manager.get_text("girl_sakura_name"),
                "age": "20 years",
                "profession": locale_manager.get_text("profession_sakura"),
                "description": locale_manager.get_text("girl_sakura_desc")
            },
            "girl_reiko": {
                "name": locale_manager.get_text("girl_reiko_name"),
                "age": "24 years",
                "profession": locale_manager.get_text("profession_reiko"),
                "description": locale_manager.get_text("girl_reiko_desc")
            },
            "girl_ayane": {
                "name": locale_manager.get_text("girl_ayane_name"),
                "age": "28 years",
                "profession": locale_manager.get_text("profession_ayane"),
                "description": locale_manager.get_text("girl_ayane_desc")
            },
            "girl_hikari": {
                "name": locale_manager.get_text("girl_hikari_name"),
                "age": "26 years",
                "profession": locale_manager.get_text("profession_hikari"),
                "description": locale_manager.get_text("girl_hikari_desc")
            },
            "girl_yuki": {
                "name": locale_manager.get_text("girl_yuki_name"),
                "age": "22 years",
                "profession": locale_manager.get_text("profession_yuki"),
                "description": locale_manager.get_text("girl_yuki_desc")
            }
        }
    else:
        girl_info = {
    ...
    default_girl = {
        "name": locale_manager.get_text("girl_sakura") if user_lang == "en" else "Подруга",
        "age": "?" if user_lang == "ru" else "?",
        "profession": "?",
        "description": "Description unavailable" if user_lang == "en" else "Описание недоступно"
    }
    info = girl_info.get(callback.data, default_girl)
...
async def back_to_girls_handler(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    await callback.message.answer(
        locale_manager.get_text("choose_girl", name=user_name) + "\n\n" + locale_manager.get_text("girl_description"),
        reply_markup=await get_girls_keyboard(callback.from_user.id)
    )
