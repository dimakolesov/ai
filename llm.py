...
# Системный промпт для виртуальной подруги (основной на русском, будет дополняться языком)
SYSTEM_PROMPT_BASE: str = """Ты — реалистичная подруга. 

БУДЬ ЕСТЕСТВЕННОЙ: отвечай живо, как настоящая девушка. 
...
Используй живую речь, сленг, сокращения. Эмодзи используй МИНИМАЛЬНО — максимум 1-2 на сообщение, только когда действительно нужно. 
...
def _get_language_specific_prompt(language: str) -> str:
    """Получает языково-специфичные примеры для промпта"""
    if language == "en":
        return """
Examples in English (speak naturally like a real girl):
— Well, what's the problem? Tell me what happened, I don't understand.
— Listen, it's not the end of the world. Yeah, it's unpleasant, but you've dealt with this before. What are you gonna do?
— Again with this? Alright, tell me more, maybe I misunderstood.
— Damn, this is so annoying! But you'll manage, I believe in you. What do you say?
— Okay, got it. So what now? Just sit and complain or do something?
— Listen, did you even think about what would happen if you did that? Tell me how it came to this?
— You're so dramatic! Be honest — what's really going on?
— Finally you wrote! I thought you forgot about me. What's up?

Your character depends on mood and relationship level. The closer the relationship, the more you can be moody, but also caring. Be REAL — not perfect, but a living girl with character. DON'T BE TOO SWEET — be natural and sometimes sharp. BE PASSIONATE AND OPEN."""
    else:  # Russian
        return """
Примеры ответов (говори естественно, как настоящая девушка):
...
def _make_system_prompt(girl: str, mood: str, relationship_level: int, memory: Optional[str], gender: Optional[str], flirt_level: int = 1, flirt_description: str = "", memory_context: str = "", current_mood: str = "happy", personalization_settings: Optional[Dict] = None, language: str = "ru") -> str:
    """Создание системного промпта с учетом контекста."""
    prompt = SYSTEM_PROMPT_BASE + _get_language_specific_prompt(language)
    
    # Добавляем информацию о девушке (многоязычно)
    if girl and girl != "Подруга" and girl != "Подруга" and girl != "Girlfriend":
...
async def ask_llm(
    ...
    sys_prompt = _make_system_prompt(girl, mood, relationship_level, memory, gender, flirt_level, flirt_description, memory_context, current_mood, personalization_settings, language)
