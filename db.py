    ...
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        await db.execute(
            ...
            CREATE TABLE IF NOT EXISTS access (
                ...
                type TEXT CHECK(type IN ('trial','paid','lifetime')) NOT NULL,
        ...
        # personalization: настройки персонализации виртуальной подруги
        await db.execute(
            """
...
async def grant_access(user_id: int, days: int, access_type: str = "trial") -> Optional[Dict[str, Any]]:
    """Предоставить доступ пользователю."""
    async with aiosqlite.connect(DB_PATH) as db:
        # Проверяем, есть ли у пользователя lifetime доступ
        if access_type != "lifetime":
            async with db.execute(
                "SELECT type FROM access WHERE user_id = ? AND type = 'lifetime'",
                (user_id,)
            ) as cur:
                if await cur.fetchone():
                    # У пользователя уже есть lifetime доступ
                    return {
                        'has_access': True,
                        'access_type': 'lifetime',
                        'message': 'Вы уже имеете пожизненный доступ'
                    }
        
        # Обработка lifetime доступа
        if access_type == "lifetime":
            await db.execute(
                """
                INSERT INTO access(user_id, expires_at, type)
                VALUES(?, NULL, ?)
                ON CONFLICT(user_id) DO UPDATE SET expires_at=NULL, type=?
                """,
                (user_id, access_type, access_type),
            )
            await db.commit()
            return {
                'has_access': True,
                'access_type': 'lifetime',
                'message': 'Пожизненный доступ активирован'
            }
        
        # Обработка временного доступа
        now = datetime.utcnow()
...
async def grant_lifetime_access(user_id: int) -> None:
    """Предоставить пожизненный доступ пользователю."""
    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute(
            """
            INSERT INTO access(user_id, expires_at, type)
...
async def has_access(user_id: int) -> bool:
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        async with db.execute(
            "SELECT expires_at, type FROM access WHERE user_id = ?", (user_id,)
        ) as cur:
            row = await cur.fetchone()
            if not row or not row[0]:
                return False
            # Lifetime access не истекает
            if row[1] == 'lifetime':
                return True
            try:
...
# ==================== ФУНКЦИИ ТРЕКИНГА ЕЖЕДНЕВНЫХ СООБЩЕНИЙ ====================

async def check_daily_message_limit(user_id: int) -> Tuple[bool, int, int]:
    """
    Проверяет, достиг ли пользователь лимита ежедневных сообщений.
    
    Returns:
        Tuple[can_send, current_count, limit]: (может ли отправить, текущее количество, лимит)
    """
    from datetime import date
    
    today = date.today()
    async with aiosqlite.connect(DB_PATH) as db:
        # Получаем текущее количество сообщений и дату последнего сообщения
        async with db.execute(
            "SELECT daily_messages_count, last_message_date FROM stats WHERE user_id = ?",
            (user_id,)
        ) as cur:
            row = await cur.fetchone()
            
            if not row or not row[0]:
                # Первое сообщение или нет записи
                daily_count = 0
                last_date = None
            else:
                daily_count = row[0]
            ...
            if last_date and last_date != str(today):
                await db.execute(
                    "UPDATE stats SET daily_messages_count = 1, last_message_date = ? WHERE user_id = ?",
                    (str(today), user_id)
                )
                await db.commit()
                daily_count = 1
            
            # Проверяем лимит (15 сообщений для пробного периода)
            limit = 15
            can_send = daily_count < limit
            
            return can_send, daily_count, limit

async def increment_daily_message_count(user_id: int) -> None:
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        async with db.execute(
            "SELECT daily_messages_count FROM stats WHERE user_id = ?",
            (user_id,)
        ) as cur:
            row = await cur.fetchone()
            
            if row:
                new_count = row[0] + 1 if row[0] else 1
                await db.execute(
                    "UPDATE stats SET daily_messages_count = ?, last_message_date = ? WHERE user_id = ?",
                    (new_count, str(today), user_id)
                )
            else:
                await db.execute(
                    "INSERT INTO stats (user_id, daily_messages_count, last_message_date) VALUES (?, 1, ?)",
                    (user_id, str(today))
                )
            
            await db.commit()

async def reset_daily_message_count(user_id: int) -> None:
    """Сбрасывает счетчик ежедневных сообщений."""
    from datetime import date
    today = date.today()
    
    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute(
            "UPDATE stats SET daily_messages_count = 0, last_message_date = ? WHERE user_id = ?",
            (str(today), user_id)
        )
        await db.commit()
