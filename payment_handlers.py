        ...
        if not payment_result["success"]:
            await callback.message.edit_text(
                ...
                reply_markup=InlineKeyboardMarkup(
                    inline_keyboard=[
                        [InlineKeyboardButton(text="⬅️ Назад", callback_data="premium_subscription")]
                    ]
                )
            )
            await callback.answer()
...
"""
        ...
        keyboard = InlineKeyboardMarkup(
            inline_keyboard=[
                [InlineKeyboardButton(text="💳 Оплатить", url=payment_result.get('payment_url', '#'))],
...
async def check_payment_handler(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    try:
        # Проверка статуса платежа недоступна для старых платежей
        status_message = "Статус платежа недоступен для этой версии бота."
        ...
        await callback.message.edit_text(
            ...
            reply_markup=InlineKeyboardMarkup(
                inline_keyboard=[
                    ...
                    [InlineKeyboardButton(text="💳 Оплатить снова", callback_data="plan_premium_month")],
                    [InlineKeyboardButton(text="⬅️ Назад", callback_data="premium_subscription")]
                ]
            )
        )
        ...
        bot_logger.log_system_error(e, f"Payment status check failed for user {callback.from_user.id}")
        await callback.message.edit_text(
            f"❌ Произошла ошибка при проверке платежа: {e}",
            reply_markup=InlineKeyboardMarkup(
                inline_keyboard=[
                    [InlineKeyboardButton(text="⬅️ Назад", callback_data="premium_subscription")]
                ]
